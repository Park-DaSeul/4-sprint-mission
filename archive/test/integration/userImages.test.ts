...
import type { User, UserImage } from '@prisma/client';
...
describe('User Image API 통합 테스트', () => {
  let user1: User;
  ...
  // 사용자 사진 생성 (업로드)
  describe('POST /userImages', () => {
    it('인증된 사용자는 사진 업로드에 성공해야 합니다 (201 Created)', async () => {
      const response = await request(app).post(`/userImages`).set('Cookie', user1Cookies).send();
      ...
      const dbImage = await prisma.userImage.findFirst({
        where: { id: userImage.id },
      });

      expect(dbImage).not.toBeNull();
      expect(dbImage.id).toBe(userImage.id);
    ...
    it('인증안된 사용자는 사진 업로드에 실패해야 합니다 (401 Unauthorized)', async () => {
      const response = await request(app).post(`/userImages`).send();
    ...
    it('DB 저장 실패 시, Cloudinary에 업로드된 이미지를 삭제(롤백)해야 합니다 (500 Internal Server Error)', async () => {
      // ---  준비 (Arrange) ---
      // '몽키 패치'를 사용하여 prisma.userImage.create를 일부러 에러 나게함
      const createMock = vi.fn().mockRejectedValue(new Error('DB 저장 강제 실패'));
      prisma.userImage.create = createMock;
